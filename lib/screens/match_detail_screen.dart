...
// 👑 نماذج البيانات الحقيقية (خالية من التجريب)
class MatchEventModel {
  ...
  const MatchEventModel({
    required this.id, required this.minute, required this.type,
    required this.playerName, required this.detail, required this.isHomeTeam,
  });
}

class MatchStatsModel {
  final int homePossession, awayPossession, homeShots, awayShots, homeShotsOnTarget, awayShotsOnTarget, homeCorners, awayCorners;
  const MatchStatsModel({
    this.homePossession = 50, this.awayPossession = 50,
    this.homeShots = 0, this.awayShots = 0,
...
class PlayerLineupModel {
  ...
  const PlayerLineupModel({
    required this.name, required this.number, required this.rating,
    required this.position, required this.xGrid, required this.yGrid,
  });
}

class MatchCommentModel {
  final String id, userName, text;
  final String? audioUrl, parentId;
  final int likes;
  final DateTime createdAt;
  MatchCommentModel({
    required this.id, required this.userName, required this.text,
    this.audioUrl, this.likes = 0, required this.createdAt, this.parentId,
  });
...
class MatchDetailScreen extends StatefulWidget {
  ...
  @override State<MatchDetailScreen> createState() => _MatchDetailScreenState();
}
...
class _MatchDetailScreenState extends State<MatchDetailScreen> with TickerProviderStateMixin {
  ...
  @override void initState() {
    ...
    if(user != null) _currentUser = user.id;
  }
  ...
  void _syncVotesFromCloud() async {
    try {
      final response = await supabase.from('match_votes').select().eq('match_id', widget.matchId).maybeSingle();
  ...
  bool _checkAndApplyBotGuard(String text, String userName) {
    ...
    if (cleanText.contains('http://') || cleanText.contains('https://') || cleanText.contains('.com') || cleanText.contains('www.')) return true;
    ...
    if (_isRecording) {
      ...
      if (path != null) {
        ...
        try {
          ...
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('فشل رفع التسجيل الصوتي', style: GoogleFonts.cairo())));
        }
      }
    } else {
      _isRecording = true;
      try {
        ...
        _isRecording = false;
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('تعذر الوصول إلى الميكروفون', style: GoogleFonts.cairo())));
      }
    }
  }
  ...
  Future<void> _submitComment(String? text, [String? audioUrl]) async {
    final user = supabase.auth.currentUser;
    if (user == null) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('يجب عليك تسجيل الدخول أولاً للتعليق', style: GoogleFonts.cairo())));
      return;
    }
    ...
    if (_checkAndApplyBotGuard(commentText, userName)) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('تم حظر الرسالة لاحتواءها على محتوى غير لائق', style: GoogleFonts.cairo())));
      return;
    }
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('حدث خطأ أثناء الإرسال', style: GoogleFonts.cairo())));
    }
  }
  ...
  Future<void> _likeComment(String commentId, int currentLikes) async {
    final user = supabase.auth.currentUser;
    if (user == null) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('يجب تسجيل الدخول للإعجاب', style: GoogleFonts.cairo())));
      return;
    }
    try {
      await supabase.from('match_comments').update({'likes': currentLikes + 1}).eq('id', commentId);
    } catch (e) { print('Error liking comment: $e'); }
  }

  // 🏟️ دالة بناء مجسم الملعب (تم ضغطها وتنسيقها)
  Widget _buildPerspectivePitch(String teamName, List<PlayerLineupModel> players) {
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 20, vertical: 8),
      decoration: BoxDecoration(color: const Color(0xFF161926).withOpacity(0.4), borderRadius: BorderRadius.circular(24), border: Border.all(color: const Color(0xFF00A3FF).withOpacity(0.2), width: 1.5)),
      child: ClipRRect(
        borderRadius: BorderRadius.circular(24),
        child: Stack(children: [
          Transform(transform: Matrix4.identity()..setEntry(3, 2, 0.002)..rotateX(-0.35), alignment: Alignment.center, child: Container(margin: const EdgeInsets.all(12), decoration: BoxDecoration(color: const Color(0xFF0C1424), borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0xFF00A3FF).withOpacity(0.3), width: 2)), child: Stack(children: [
            Center(child: Container(height: 1, width: double.infinity, color: const Color(0xFF00A3FF).withOpacity(0.15))),
            Center(child: Container(width: 90, height: 90, decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: const Color(0xFF00A3FF).withOpacity(0.15), width: 1.5)))),
            Center(child: Text('NF', style: GoogleFonts.cairo(color: const Color(0xFF00A3FF).withOpacity(0.12), fontSize: 24, fontWeight: FontWeight.w900, letterSpacing: 2)))],
          ))),
          Positioned(top: 14, left: 0, right: 0, child: Center(child: Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 4), decoration: BoxDecoration(color: const Color(0xFF0F111A).withOpacity(0.85), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFF00A3FF).withOpacity(0.3), width: 0.8)), child: Text(teamName, style: GoogleFonts.cairo(color: Colors.white, fontSize: 11, fontWeight: FontWeight.bold))))),
          ...players.map((player) {
            final double alignmentX = -1.0 + ((player.xGrid - 1) * 0.5);
            final double alignmentY = 0.85 - ((player.yGrid - 1) * 0.42);
            return Align(alignment: Alignment(alignmentX, alignmentY), child: Column(mainAxisSize: MainAxisSize.min, children: [
              Stack(clipBehavior: Clip.none, children: [
                Container(width: 32, height: 32, decoration: BoxDecoration(color: const Color(0xFF0F111A), shape: BoxShape.circle, border: Border.all(color: const Color(0xFF00A3FF), width: 1.5), boxShadow: [BoxShadow(color: const Color(0xFF00A3FF).withOpacity(0.3), blurRadius: 6)]), child: Center(child: Text(player.number, style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.bold)))),
                Positioned(top: -3, right: -6, child: Container(padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 1), decoration: BoxDecoration(color: const Color(0xFF00A3FF), borderRadius: BorderRadius.circular(4)), child: Text(player.rating, style: const TextStyle(color: Colors.white, fontSize: 7, fontWeight: FontWeight.bold))))
              ]),
              const SizedBox(height: 3),
              Container(padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 1), decoration: BoxDecoration(color: const Color(0xFF0F111A).withOpacity(0.75), borderRadius: BorderRadius.circular(4)), child: Text(player.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: GoogleFonts.cairo(color: Colors.white, fontSize: 8, fontWeight: FontWeight.bold)))
            ]));
          })
        ]),
      ),
    ...
    // 🔄 دالة بناء الخط الزمني (تم تنظيفها بالكامل من البيانات الوهمية)
  Widget _buildTimelineEventsSection() {
    // 🔮 ملاحظة: هنا يتم ترك مصفوفة الأحداث مرتبطة باسم السحابة مستقبلاً.
    // في الإصدار القادم، سنستبدل هذه القائمة بـ StreamBuilder يستمع لجدول match_events.
    return Padding(
      ...
      child: Container(
        ...
        child: GlassCard(
          ...
          child: ListView.builder(
            ...
            itemCount: 0, // تم تصفير العداد لإزالة البيانات الوهمية
          ...
          SingleChildScrollView(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              StreamBuilder<List<Map<String, dynamic>>>(
                stream: supabase.from('match_comments').stream(primaryKey: ['id']).eq('match_id', widget.matchId).order('created_at', ascending: false).map((data) => List<Map<String, dynamic>>.from(data)),
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: Padding(padding: EdgeInsets.all(20), child: CircularProgressIndicator(color: Color(0xFF00A3FF))));
                  final commentsData = snapshot.data ?? [];
                  if (commentsData.isEmpty) return Padding(padding: const EdgeInsets.symmetric(vertical: 40), child: Center(child: Text('كن أول من يطلق العنان لصوته في هذه الملحمة!', style: GoogleFonts.cairo(color: Colors.white38, fontSize: 11, fontWeight: FontWeight.bold))));
                  return ListView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), itemCount: commentsData.length, itemBuilder: (context, index) {
              ...
              Padding(padding: const EdgeInsets.all(20), child: Row(textDirection: TextDirection.rtl, children: [
                ...
                AnimatedBuilder(animation: _audioRecorder, builder: (context, _) {
                  return GestureDetector(onLongPressStart: (_) => _toggleRecording(), onLongPressEnd: (_) => _toggleRecording(), child: Container(decoration: BoxDecoration(color: _isRecording ? Colors.redAccent.withOpacity(0.2) : const Color(0xFF161926), shape: BoxShape.circle, border: Border.all(color: _isRecording ? Colors.redAccent : Colors.white10)), child: Padding(padding: const EdgeInsets.all(8), child: _isRecording ? const Icon(Icons.stop_rounded, color: Colors.white, size: 18) : const Icon(Icons.mic_rounded, color: Colors.white60, size: 18))));
                }),
